...
def test_sync_platforms_tolerates_missing_cache_extension(monkeypatch):
    ...
    monkeypatch.setattr(
        profile_routes,
        "run_fetch_jobs",
        lambda fetch_jobs, max_workers=4: ({"gfg": {"total": 77}}, {}),
    )

    response = app.test_client().post("/sync_platforms", json={"gfg": "gfg-user"})

    payload = response.get_json()

    assert response.status_code == 200
    assert payload["success"] is True
    assert payload["platforms"]["gfg"]["status"] == "synced"
    assert captured["db_update"][1]["$set"]["external_totals"]["GFG"] == 77
