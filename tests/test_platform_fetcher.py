...
from app.platforms.fetchers import clear_platform_http_session, fetch_atcoder, fetch_spoj, run_fetch_jobs
...
def test_fetch_spoj_returns_total_on_success():
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.text = '<html><body><div>Solved problems: 123</div></body></html>'
    fake_session = MagicMock()
    fake_session.get.return_value = mock_response

    with patch('app.platforms.fetchers._get_http_session', return_value=fake_session):
        result = fetch_spoj('tourist')
